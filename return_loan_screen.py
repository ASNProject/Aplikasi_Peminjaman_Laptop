...
t24 = Label(root, text='NOTA PENGEMBALIAN', font=("Arial bold", 14))
...
t43 = Label(root, fg="black", bg="white", text='JATUH TEMPO', font=("Arial", 12))
    ...
    if check_member == "AKTIF":
        try:
            ...
            try:
                datas = (val,)
                print(datas)
                qs = "SELECT * FROM peminjaman_data WHERE laptop_id= ?"
                cursors = connection.execute(qs, datas)
                data_rows = cursors.fetchmany(rfid)
                for rows in data_rows:
                    date = dt.datetime.now()
                    format_date = f"{date:%d %b %Y}"
                    np_tanggalkembali.set(format_date)
                    np_tanggalpinjam.set(rows[7])
                    print(rows[7])
            except sqlite3.Error as error:
                print("error:", error)
        except:
...
def upload_data():
    ...
    cursor.execute("DELETE FROM peminjaman_data WHERE rfid_member= ?", (int(t7.getvar(t7.cget("textvariable"))),))
    cursor.execute("UPDATE member_data SET status=? WHERE rfid=?",
                   ('NOT LOAN', int(t7.getvar(t7.cget("textvariable")))))
    cursor.execute("UPDATE laptop_data SET status=? WHERE rfid=?",
                   ('NOT LOAN', int(t17.getvar(t17.cget("textvariable")))))
    connection.commit()
    done_upload()
    # ids.set("")
    # nama.set("")
